...
class TransactionController extends Controller
{
  ...
  public function index(Request $request)
  {
        ...
        $model_query = $model_query->where(function ($q)use($like_lists){
          ...
          if (isset($like_lists["warehouse_name"])) {
            $q->orWhereIn("hrm_revisi_lokasi_id", function($q2)use($like_lists) {
              ...
              ->select('id')->where("name",$like_lists['warehouse_name']);          
            });
          }
          ...
          if (isset($like_lists["warehouse_source_name"])) {
            $q->orWhereIn("hrm_revisi_lokasi_source_id", function($q2)use($like_lists) {
              $q2->from('hrm_revisi_lokasi')
              ->select('id')->where("name",$like_lists['warehouse_source_name']);          
            });
          }
          ...
          if (isset($like_lists["warehouse_target_name"])) {
            $q->orWhereIn("hrm_revisi_lokasi_target_id", function($q2)use($like_lists) {
              $q2->from('hrm_revisi_lokasi')
              ->select('id')->where("name",$like_lists['warehouse_target_name']);          
            });
          }
          ...
          if (isset($like_lists["item_name"])) {
            $q->orWhereIn("st_item_id", function($q2)use($like_lists) {
              $q2->from('st_items')
              ->select('id')->where("name",$like_lists['item_name']);          
            });
          }
          ...
          if (isset($like_lists["requested_name"])) {
            $q->orWhereIn("requested_by", function($q2)use($like_lists) {
              $q2->from('is_users')
              ->select('id_user')->where("username",$like_lists['requested_name']);          
            });
          }
    
          if (isset($like_lists["approved_name"])) {
            $q->orWhereIn("approved_by", function($q2)use($like_lists) {
              $q2->from('is_users')
              ->select('id_user')->where("username",$like_lists['approved_name']);          
            });
          }
        });        
      }
    ...
    $model_query = $model_query->with(['warehouse','warehouse_source','warehouse_target','requester', 'approver',
    'item'=>function($q){
      $q->with('unit');      
    }])->get();
  ...
  public function show(TransactionRequest $request)
  {
    ...
    $model_query = Transaction::with(['warehouse','warehouse_source','warehouse_target','requester', 'approver',
    'item'=>function($q){
      $q->with('unit');      
    }])->find($request->id);
  ...
  public function store(TransactionRequest $request)
  {
    ...
    try {
      ...
      $model_query->approved_at               = date("Y-m-d H:i:s");
      $model_query->approved_by               = $this->admin_id;
